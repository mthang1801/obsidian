...
> Lane này gom lại những quyết định type nhỏ nhưng sống dai nhất trong Java codebase.

📅 Ngày tạo: 2026-03-27 · 🔄 Cập nhật: 2026-04-04 · ⏱️ 5 phút đọc

## 1. DEFINE

Hình dung một bug rất nhỏ nhưng cứ quay lại mãi: so sánh wrapper bằng `==`, để `null` trôi qua ba lớp, hoặc dùng `double` cho tiền. Những quyết định type này không ồn ào, nhưng chúng để lại vết nứt rất sâu nếu bị xem nhẹ.

Lane `Java Fundamentals Types` được mở để biến một đường link từng chết thành một decision point có thể dùng thật trong reading path của `assets/java`.
