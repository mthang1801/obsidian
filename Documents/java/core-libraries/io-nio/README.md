...
# Java Core Libraries IO & NIO

> Lane này kéo IO và NIO về đúng mental model: path là địa chỉ, files là facade, channel/buffer là data movement.

📅 Ngày tạo: 2026-03-27 · 🔄 Cập nhật: 2026-04-04 · ⏱️ 5 phút đọc

## 1. DEFINE

Hình dung một đoạn code xử lý file trông rất nhỏ nhưng lại là nơi bug path, leak resource hoặc memory pressure bắt đầu nhen lên. IO chỉ nguy hiểm khi nó bị xem như việc phụ. NIO chỉ hữu ích khi bạn hiểu mình đang cầm path, file operation hay byte movement.

Lane `Java Core Libraries IO & NIO` được mở để biến một đường link từng chết thành một decision point có thể dùng thật trong reading path của `assets/java`.
...
| --- | --- | --- |
| [`../http-client/README.md`](../http-client/README.md) | Khi lane này chưa đủ để mở hết tension | Giữ người đọc ở cùng cụm thay vì nhảy ngẫu nhiên |
| [`../serialization/README.md`](../serialization/README.md) | Khi lane này chưa đủ để mở hết tension | Giữ người đọc ở cùng cụm thay vì nhảy ngẫu nhiên |
| [`../streams/README.md`](../streams/README.md) | Khi lane này chưa đủ để mở hết tension | Giữ người đọc ở cùng cụm thay vì nhảy ngẫu nhiên |
