...
> Router cho nhánh Java Performance, nơi Allocation, Benchmarking, Profiling và các lane liên quan được ghép thành một đường học có chủ đích.

📅 Ngày tạo: 2026-03-27 · 🔄 Cập nhật: 2026-04-04 · ⏱️ 5 phút đọc

## 1. DEFINE

Hình dung một service Java bắt đầu chậm đi nhưng không đủ chậm để chỉ ra ngay thủ phạm. Người thì đòi chỉnh JVM flag, người muốn rewrite một đoạn code nóng, người khác lại tin rằng chỉ cần scale thêm máy. Nếu không có baseline và đường điều tra rõ ràng, performance work sẽ biến thành một chuỗi placebo rất tốn thời gian.

Riêng với nhánh `Java Performance`, mục tiêu không phải đọc cho đủ mục. Mục tiêu là biết nên vào từ đâu khi symptom, câu hỏi hoặc decision point đã xuất hiện ngay trong công việc thật.
...
| --- | --- | --- |
| [`allocation/`](allocation/README.md) | Directory | Router cho nhánh Allocation |
| [`benchmarking/`](benchmarking/README.md) | Directory | Router cho nhánh Benchmarking |
| [`profiling/`](profiling/README.md) | Directory | Router cho nhánh Profiling |
| [`tuning/`](tuning/README.md) | Directory | Router cho nhánh Tuning |
...
```text
Allocation -> Benchmarking -> Profiling -> Tuning
```
...
----------------------------  ----------------------------  ----------------
Allocation                    entry via README               0 article(s)
Benchmarking                  entry via README               1 article(s)
Profiling                     entry via README               1 article(s)
Tuning                        entry via README               0 article(s)
```
...
| --- | --- | --- |
| [`allocation/`](./allocation/README.md) | Khi muốn đi sâu vào Allocation | Giữ learning path liền mạch |
| [`benchmarking/`](./benchmarking/README.md) | Khi muốn đi sâu vào Benchmarking | Giữ learning path liền mạch |
| [`profiling/`](./profiling/README.md) | Khi muốn đi sâu vào Profiling | Giữ learning path liền mạch |
| [`tuning/`](./tuning/README.md) | Khi muốn đi sâu vào Tuning | Giữ learning path liền mạch |
