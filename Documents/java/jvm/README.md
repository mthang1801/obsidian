...
> Router cho nhánh Java JVM, nơi Class Loading, Garbage Collection, JIT và các lane liên quan được ghép thành một đường học có chủ đích.

📅 Ngày tạo: 2026-03-27 · 🔄 Cập nhật: 2026-04-04 · ⏱️ 5 phút đọc

## 1. DEFINE

Hình dung dashboard vừa đổi màu: GC pause dài hơn, startup lạ đi, memory giữ mãi không nhả, hoặc benchmark local đẹp nhưng production vẫn ì ạch. Từ khoảnh khắc đó trở đi, bạn không còn được sống chỉ bằng intuition ở tầng code nữa. Bạn phải nhìn JVM như một phần của câu chuyện vận hành.

Riêng với nhánh `Java JVM`, mục tiêu không phải đọc cho đủ mục. Mục tiêu là biết nên vào từ đâu khi symptom, câu hỏi hoặc decision point đã xuất hiện ngay trong công việc thật.
...
| --- | --- | --- |
| [`class-loading/`](class-loading/README.md) | Directory | Router cho nhánh Class Loading |
| [`gc/`](gc/README.md) | Directory | Router cho nhánh Garbage Collection |
| [`jit/`](jit/README.md) | Directory | Router cho nhánh JIT |
| [`memory-model/`](memory-model/README.md) | Directory | Router cho nhánh Memory Model |
...
```text
Class Loading -> Garbage Collection -> JIT -> Memory Model
```
...
----------------------------  ----------------------------  ----------------
Class Loading                 entry via README               0 article(s)
Garbage Collection            entry via README               1 article(s)
JIT                           entry via README               0 article(s)
Memory Model                  entry via README               0 article(s)
```
...
| --- | --- | --- |
| [`class-loading/`](./class-loading/README.md) | Khi muốn đi sâu vào Class Loading | Giữ learning path liền mạch |
| [`gc/`](./gc/README.md) | Khi muốn đi sâu vào Garbage Collection | Giữ learning path liền mạch |
| [`jit/`](./jit/README.md) | Khi muốn đi sâu vào JIT | Giữ learning path liền mạch |
| [`memory-model/`](./memory-model/README.md) | Khi muốn đi sâu vào Memory Model | Giữ learning path liền mạch |
