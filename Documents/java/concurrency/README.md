...
> Router cho nhánh Java Concurrency, nơi Basics, Completable Future, Concurrent Collections và các lane liên quan được ghép thành một đường học có chủ đích.

📅 Ngày tạo: 2026-03-27 · 🔄 Cập nhật: 2026-04-04 · ⏱️ 5 phút đọc

## 1. DEFINE

Hình dung một batch đêm qua không hẳn sập nhưng chạy chậm bất thường: hàng đợi dài ra, thread pool nghẹt dần, còn log thì không đủ thẳng thắn để chỉ ngay bug nằm ở race, ở blocking hay ở mô hình phối hợp. Khi concurrency bắt đầu hiện hình như vậy, bạn cần nhiều hơn tên API. Bạn cần một đường học bám đúng symptom.

Riêng với nhánh `Java Concurrency`, mục tiêu không phải đọc cho đủ mục. Mục tiêu là biết nên vào từ đâu khi symptom, câu hỏi hoặc decision point đã xuất hiện ngay trong công việc thật.
...
| [`completable-future/`](completable-future/README.md) | Directory | Router cho nhánh Completable Future |
| [`concurrent-collections/`](concurrent-collections/README.md) | Directory | Router cho nhánh Concurrent Collections |
| [`executors/`](executors/README.md) | Directory | Router cho nhánh Executors |
| [`locks/`](locks/README.md) | Directory | Router cho nhánh Locks |
| [`virtual-threads/`](virtual-threads/README.md) | Directory | Router cho nhánh Virtual Threads |
...
```text
Basics -> Completable Future -> Concurrent Collections -> Executors -> Locks -> Virtual Threads
```
...
----------------------------  ----------------------------  ----------------
Basics                        entry via README               1 article(s)
Completable Future            entry via README               1 article(s)
Concurrent Collections        entry via README               0 article(s)
Executors                     entry via README               1 article(s)
Locks                         entry via README               0 article(s)
Virtual Threads               entry via README               1 article(s)
```
...
| [`completable-future/`](./completable-future/README.md) | Khi muốn đi sâu vào Completable Future | Giữ learning path liền mạch |
| [`concurrent-collections/`](./concurrent-collections/README.md) | Khi muốn đi sâu vào Concurrent Collections | Giữ learning path liền mạch |
| [`executors/`](./executors/README.md) | Khi muốn đi sâu vào Executors | Giữ learning path liền mạch |
| [`locks/`](./locks/README.md) | Khi muốn đi sâu vào Locks | Giữ learning path liền mạch |
| [`virtual-threads/`](./virtual-threads/README.md) | Khi muốn đi sâu vào Virtual Threads | Giữ learning path liền mạch |
