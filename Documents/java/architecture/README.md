...
> Router cho nhánh Java Architecture, nơi DDD, Hexagonal, Layered Architecture và các lane liên quan được ghép thành một đường học có chủ đích.

📅 Ngày tạo: 2026-03-27 · 🔄 Cập nhật: 2026-04-04 · ⏱️ 5 phút đọc

## 1. DEFINE

Hình dung service Java của bạn đã qua vài quý phát triển: thêm người, thêm module, thêm tích hợp, và rồi một ngày mọi thay đổi nhỏ đều đụng vào hai ba lớp không liên quan. Lúc đó, chuyện kiến trúc không còn là sơ đồ đẹp nữa. Nó trở thành câu hỏi rất thực: boundary nào đang giữ hệ thống đứng vững, và boundary nào đã rò rỉ từ lâu mà chưa ai gọi đúng tên.

Riêng với nhánh `Java Architecture`, mục tiêu không phải đọc cho đủ mục. Mục tiêu là biết nên vào từ đâu khi symptom, câu hỏi hoặc decision point đã xuất hiện ngay trong công việc thật.
...
| --- | --- | --- |
| [`ddd/`](ddd/README.md) | Directory | Router cho nhánh DDD |
| [`hexagonal/`](hexagonal/README.md) | Directory | Router cho nhánh Hexagonal |
| [`layered/`](layered/README.md) | Directory | Router cho nhánh Layered Architecture |
...
```text
DDD -> Hexagonal -> Layered Architecture
```
...
----------------------------  ----------------------------  ----------------
DDD                           entry via README               0 article(s)
Hexagonal                     entry via README               1 article(s)
Layered Architecture          entry via README               0 article(s)
```
...
| --- | --- | --- |
| [`ddd/`](./ddd/README.md) | Khi muốn đi sâu vào DDD | Giữ learning path liền mạch |
| [`hexagonal/`](./hexagonal/README.md) | Khi muốn đi sâu vào Hexagonal | Giữ learning path liền mạch |
| [`layered/`](./layered/README.md) | Khi muốn đi sâu vào Layered Architecture | Giữ learning path liền mạch |
