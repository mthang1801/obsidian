...
> Lane này tách validation theo đúng tầng: request contract, use case policy và domain invariant.

📅 Ngày tạo: 2026-03-27 · 🔄 Cập nhật: 2026-04-04 · ⏱️ 5 phút đọc

## 1. DEFINE

Hình dung một request sai đi vào hệ thống và không ai còn chắc lỗi đó thuộc về contract HTTP, policy của use case hay invariant của domain. Validation chỉ đáng học khi bạn cần tách ba lớp đó ra để error response và business rule không cắn vào nhau.

Lane `Java Backend Validation` được mở để biến một đường link từng chết thành một decision point có thể dùng thật trong reading path của `assets/java`.
