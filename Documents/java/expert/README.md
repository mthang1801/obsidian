...
> Router cho nhánh Java Expert Topics, nơi Distributed Systems, Observability, Patterns và các lane liên quan được ghép thành một đường học có chủ đích.

📅 Ngày tạo: 2026-03-27 · 🔄 Cập nhật: 2026-04-04 · ⏱️ 5 phút đọc

## 1. DEFINE

Hình dung ca trực đêm bắt đầu bằng một sự cố không thể gán cho riêng framework hay riêng ngôn ngữ: timeout nối đuôi nhau, trace đứt đoạn, retry bắt đầu khuếch đại vấn đề thay vì cứu nó. Đây là lúc các chủ đề “expert” không còn là phần đọc thêm cho vui. Chúng trở thành loại judgment mà một backend engineer phải có khi hệ thống bắt đầu chống trả lại những giải thích quá đơn giản.

Riêng với nhánh `Java Expert Topics`, mục tiêu không phải đọc cho đủ mục. Mục tiêu là biết nên vào từ đâu khi symptom, câu hỏi hoặc decision point đã xuất hiện ngay trong công việc thật.
...
| --- | --- | --- |
| [`distributed-systems/`](distributed-systems/README.md) | Directory | Router cho nhánh Distributed Systems |
| [`observability/`](observability/README.md) | Directory | Router cho nhánh Observability |
| [`patterns/`](patterns/README.md) | Directory | Router cho nhánh Patterns |
| [`resilience/`](resilience/README.md) | Directory | Router cho nhánh Resilience |
...
```text
Distributed Systems -> Observability -> Patterns -> Resilience
```
...
----------------------------  ----------------------------  ----------------
Distributed Systems           entry via README               0 article(s)
Observability                 entry via README               1 article(s)
Patterns                      entry via README               0 article(s)
Resilience                    entry via README               1 article(s)
```
...
| --- | --- | --- |
| [`distributed-systems/`](./distributed-systems/README.md) | Khi muốn đi sâu vào Distributed Systems | Giữ learning path liền mạch |
| [`observability/`](./observability/README.md) | Khi muốn đi sâu vào Observability | Giữ learning path liền mạch |
| [`patterns/`](./patterns/README.md) | Khi muốn đi sâu vào Patterns | Giữ learning path liền mạch |
| [`resilience/`](./resilience/README.md) | Khi muốn đi sâu vào Resilience | Giữ learning path liền mạch |
