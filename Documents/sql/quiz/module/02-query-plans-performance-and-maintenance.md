...
# ✅ SQL Module Quiz — Query Plans, Performance & Maintenance

> Quiz cho `assets/sql/optimizer` và `assets/sql/postgresql/performance`: planner, index selection, lock behavior, VACUUM, WAL, checkpoint và pagination.

| Aspect | Detail |
| --- | --- |
| **Level** | Intermediate → Advanced |
| **Coverage** | EXPLAIN, indexes, locks, autovacuum, WAL, pagination |
...
EXPLAIN ANALYZE cho thấy `Seq Scan on orders (rows=50000)` nhưng bảng chỉ có 500 rows match filter. Bạn biết câu trả lời: "missing index" hoặc "ANALYZE chưa chạy". Nhưng **tại sao** planner estimate sai? `n_distinct` trong `pg_stats` bị stale, hay selectivity formula đang dùng MCV list outdated?

Quiz này test khả năng đọc plan, hiểu planner decision, và biết khi nào cần index vs khi nào cần VACUUM ANALYZE — không phải thuộc syntax EXPLAIN.
...
Các failure mode trên nghe rõ. Nhưng có trap: đọc EXPLAIN output mà không hiểu cost model = optimize sai hướng. Trap đó sẽ xuất hiện ở PITFALLS.

## 2. VISUAL

Với SQL Module Quiz — Query Plans, Performance & Maintenance, điều cần nhìn trước không phải đáp án mà là cấu trúc reasoning của câu hỏi. Chỉ khi thấy nó đang kiểm tra lớp mental model nào, bạn mới tránh được việc chọn theo phản xạ.

![Quiz Knowledge Map — Plan Reading, Cost Analysis, Optimization diagnostic layers](../images/quiz-query-perf-map.png)
...
*Hình: Level 1 cho ✅ SQL Module Quiz — Query Plans, Performance & Maintenance — nhìn vào happy path hoặc baseline heuristic trước khi đi sâu vào planner và trade-off.*
...
*Hình: Level 2 biến ✅ SQL Module Quiz — Query Plans, Performance & Maintenance thành checklist quyết định — từ semantics, sang plan signal, rồi đến áp lực production.*

---
## 3. CODE

Khi pattern reasoning của SQL Module Quiz — Query Plans, Performance & Maintenance đã rõ, ta chuyển sang câu hỏi, truy vấn và artifact cụ thể để tự kiểm chứng xem mình đang hiểu cơ chế hay chỉ nhớ từ khóa.

### Problem 1: Basic — Structured artifact minh họa

> **Mục tiêu**: Minh họa cách áp dụng **✅ SQL Module Quiz — Query Plans, Performance & Maintenance** qua ví dụ `Structured artifact minh họa` trong đúng ngữ cảnh schema, query hoặc vận hành.
...
```sql
-- performance_quiz.sql — Measure before changing planner-facing structures
...
---
Bạn đã đi qua query plans quiz. Bây giờ đến phần nguy hiểm: cost model misunderstanding — trap đã được setup từ đầu bài.

## 4. PITFALLS

SQL Module Quiz — Query Plans, Performance & Maintenance đáng giá vì nó chỉ ra đúng kiểu sai lầm sẽ lặp lại trong production nếu không sửa mental model. Phần dưới đây gom những mẫu suy nghĩ dễ trượt nhất.
...
---
Bạn đã đi qua Quiz Performance và cạm bẫy. Các resources dưới đây giúp đi sâu hơn.
...
Khi đã nhìn ra mình hay sai ở đâu với SQL Module Quiz — Query Plans, Performance & Maintenance, bước tiếp theo là quay lại đúng module hoặc scenario liên quan để lấp khoảng trống đó.

| Mở rộng | Khi nào | Lý do | File/Link |
| --- | --- | --- | --- |
| Đọc lại [optimizer/README.md](../../optimizer/README.md) và [performance/README.md](../../postgresql/performance/README.md) | Khi cần mở rộng sau bài hiện tại | Giữ learning path liền mạch | Nội bộ module |
