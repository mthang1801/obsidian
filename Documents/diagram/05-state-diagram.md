...
| Aspect | Detail |
| --- | --- |
| **Concept** | Legacy bridge for `State Diagram` |
...
Picture a workflow with states like draft, approved, paid, shipped, and cancelled, but the transition rules are scattered across multiple services. A state diagram appears at the right moment: when the team needs to see the full lifecycle as a state machine instead of a pile of disconnected conditionals.
...
| New file | Role |
| --- | --- |
| [State Diagram](03-behavioral/03-state-diagram.md) | Current canonical article for this topic |
...
> **Example**: `State Diagram` has been regrouped into the appropriate subfolder.
> **Complexity**: Basic

```yaml
bridge_doc:
  old_path: 05-state-diagram.md
  canonical_path: 03-behavioral/03-state-diagram.md
...
| --- | --- | --- | --- |
| Mermaid state diagram | Official docs | https://mermaid.js.org/syntax/stateDiagram.html | State transition syntax |
| PlantUML state diagram | Official docs | https://plantuml.com/state-diagram | UML-oriented state modeling |
...
| --- | --- | --- | --- |
| State Diagram | When you need canonical content, examples, and real pitfalls | The canonical article is maintained under the new workflow | [State Diagram](03-behavioral/03-state-diagram.md) |
